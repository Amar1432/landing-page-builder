...
export const MinimalistTheme: ThemeComponents = {
  Layout: ({ children, settings }) => {
    const fontClass = settings?.fontFamily === 'serif' ? 'font-serif' : settings?.fontFamily === 'sans' ? 'font-sans' : 'font-mono';
    return (
      <div className={`min-h-screen bg-[#FDFDFD] text-stone-800 ${fontClass} tracking-tight`}>
        <header className="py-8 px-10 border-b-2 border-stone-200 flex justify-between items-center text-xs uppercase font-bold tracking-widest">
          <span>{settings?.siteName || 'MINIMALIST'}</span>
        </header>
