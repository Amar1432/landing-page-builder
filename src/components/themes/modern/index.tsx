...
export const ModernTheme: ThemeComponents = {
  ...
  Hero: ({ headline, subheadline, ctaText, ctaUrl }) => (
    <section className="py-24 px-6 text-center bg-gradient-to-b from-blue-50 to-white">
      <h1 className="text-5xl font-extrabold tracking-tight text-blue-900 mb-6">{headline}</h1>
      {subheadline && <p className="text-xl text-slate-600 max-w-2xl mx-auto mb-10">{subheadline}</p>}
      <a href={ctaUrl} className="inline-block bg-blue-600 text-white font-medium px-8 py-4 rounded-full shadow-md hover:bg-blue-700 transition">
        {ctaText}
      </a>
    </section>
  ),
  ...
  Pricing: ({ items }) => (
    <section className="py-20 px-6 bg-slate-900 text-white">
      <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-8">
        {items.map((item) => (
          <div key={item.id} className={`p-8 rounded-3xl ${item.isPopular ? 'bg-blue-600 ring-4 ring-blue-500/30 transform scale-105' : 'bg-slate-800'}`}>
